...
# --- tick momentum arms +1.0R / frac 0.50, keeps low sub-arm rungs --------------
...
def test_tick_burst_rider_peak_fraction_keeps_sub_arm_rungs() -> None:
    ...
    # Peak-fraction arm +1.0R / frac 0.50 on a 3.5-ATR wide trail.
    assert sched.peak_lock_arm_r == _TICK_PEAK_LOCK_ARM_R == 1.0
...
def test_tick_flow_pressure_peak_fraction_armed() -> None:
    ...
    # Same peak-fraction arm; the wide flow_pressure trail (4.0) is unchanged.
    assert sched.peak_lock_arm_r == _TICK_PEAK_LOCK_ARM_R == 1.0
    assert sched.peak_lock_frac == _TICK_PEAK_LOCK_FRAC == 0.50
