...
(function () {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const rj = 0.78 + Math.random() * 0.22;
    x *= rj; y *= rj; z *= rj;
  ...
  const firing = [];
  const FIRING_TARGET = 28; // average # of edges firing at any time
  ...
  function drawCloudSphereGlow() {
    ...
    g.addColorStop(0,   'rgba(95,135,175,0.18)');
    g.addColorStop(0.5, 'rgba(95,135,175,0.05)');
  ...
  function drawDormantEdges(projected) {
    // Render every edge as a barely-visible thread so the topology is felt
    ctx.strokeStyle = 'rgba(95,135,175,0.045)';
    ctx.lineWidth = 0.5;
    ctx.beginPath();
    for (const e of edges) {
      const a = projected[e.a], b = projected[e.b];
      // Skip back-side strands to avoid visual noise
      if (a.depth < -0.2 && b.depth < -0.2) continue;
