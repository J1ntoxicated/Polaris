...
**다음 세션 착수(워크플로우)**: Phase2/3/hardening/대시보드단계2 완료(972 green, 6 커밋 `cf98ed8`→`2c549af`). ⏳ **#9 P0 버그 FIX 진행중**(`wzqg6u54b`: OKX fill/same-bar/Capital payload/G6/instrumentation diagnose→fix→review→gate; ⚠️SL·IOC 파라미터는 silent 변경 X /debate 플래그) → **#26 엑싯 정밀도**(G7 rich-context+MFE/MAE counterfactual+tiered trailing FSM, auto_invasion exit_fsm 참조) → **alt-data regime**(auto_invasion 가중스코어 engine+provider layer[F&G/funding/OI/VIX/sentiment]+fusion+Bayesian calib, signal not throttle) → **#10 AI 효율화**(G1 determ default/G3·G4 OKX python pre-check/prompt cap) → 비용 모니터링 → **최종**: ⚠️3-스트림 graceful 재기동(PID67774 **SIGINT**[SIGTERM 핸들러 없음]→오펀 체크→새 코드, 첫 실 Alpaca paper fetch_account 선검증·실패 시 청산)+교차 live-audit(cross-contam 0)+5-axis. task #1-10 추적.
...
- P1.0 ignition fired at 2026-05-29 18:27 (paper=False, full_pipeline=True)
