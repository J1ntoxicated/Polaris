...
- Every closed trade still emits a deterministic REFLECTED lesson + clamped Δ
  and persists an ``ai_lessons`` row + a vault markdown file.
"""
...
async def test_g8_lesson_markdown_write(
    ...
    """Deterministic lesson is written to vault/50_research/lessons/<trade>_<date>.md."""
    closed_trade = {
        "trade_id": "trade-md",
        "strategy_id": "vb",
        "regime": "bull_trend",
    ...
    # Deterministic template text for a +2.0R winner.
    assert "keep cell routing weight" in content
...
async def test_g8_soft_mode_dampens_delta(
    memdb: sqlite3.Connection, tmp_path: Path
) -> None:
...
async def test_g8_no_closed_trade_is_noop(
    memdb: sqlite3.Connection, tmp_path: Path
) -> None:
    ctx = _ctx({})
    result = await post_trade_reflector_gate(
