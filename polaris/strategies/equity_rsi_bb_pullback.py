"""Equity RSI-BB Pullback — Alpaca US equity (Track C, T12).

Re-skin of :class:`~polaris.strategies.rsi_bb_pullback.RSIBBPullbackStrategy`
for the additive US-equity stream (StreamConfig C_alpaca_equity). Large-cap
dip-buy inside an uptrend, long-only (P0). Same trigger logic as the OKX
strategy; only venue / asset_class / correlation_group differ — the OKX
RSI-BB strategy is untouched.
...
# Strength curve (frozen v1 — mirrors OKX RSI-BB).
STRENGTH_FLOOR = 0.4
STRENGTH_OFFSET = 0.5
TTL_BARS = 4


class EquityRSIBBPullbackStrategy(BaseStrategy):
    ...
    metadata = StrategyMetadata(
        ...
        asset_class="equity",
        venue="alpaca",
        correlation_group_id="equity_mean_reversion",
    ...
    def generate_raw_signal(self, market_view: MarketView) -> RawSignal | None:
        ...
        return RawSignal(
            ...
            thesis_tag=f"equity_rsi={rsi:.1f}<30+bb_lo+ma200",
