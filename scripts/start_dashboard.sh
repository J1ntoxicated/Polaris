#!/bin/bash
# Polaris Dashboard launcher — WORK/OFFHOURS profile + fixed cols×rows
#
# 모태 invasion start.sh 패턴 (Jin 2026-04-13 mandate):
#   🟢 WORK     주중 09:00~17:00     INTEL @ (1913, 30)   — 우측 모니터 top-left (3 monitor 정렬)
#   🟦 OFFHOURS 그 외 (저녁/주말)    INTEL @ (3465, 30)   — 우측 LG monitor (확장 layout)
#
# Window cell size = 180 cols × 50 rows (Jin "컬럼 폭/줄수 고정" mandate).
# Refresh = 1s (Jin "1초 실시간" mandate).
#
# Override:
#   POLARIS_DASH_PROFILE=WORK|OFFHOURS  (시간 자동 감지 override)
#   POLARIS_DASH_POSITION="x, y"        (window top-left 좌표)
#   POLARIS_DASH_REFRESH=2              (refresh interval seconds)
#   ./start_dashboard.sh --inline       (현재 terminal에서 실행)

set -e

POLARIS_DIR="/Users/jinyoon/Projects/Polaris"
REFRESH_SEC="${POLARIS_DASH_REFRESH:-1}"
...
if [[ "$PROFILE" == "WORK" ]]; then
    BOUNDS_DEFAULT="1913, 30, 3833, 1069"
else
    BOUNDS_DEFAULT="3465, -1050, 5382, -11"
fi
...
pkill -f "src.dashboard.cli" 2>/dev/null || true
sleep 0.5

# ── 1b. Close existing Polaris Dashboard / orphan shell Terminal windows ───
# (always — lightweight, only matches Polaris/orphan shell titles)
...
tell application "Terminal"
    ...
    repeat while i > 0
        try
            ...
            if (tt starts with "Polaris Dashboard") or (tt starts with "jinyoon@") or (tt contains "src.dashboard.cli") or (tt contains "polaris_dashboard.command") then
                close w saving no
            end if
        end try
        set i to i - 1
...
# do script 반환 tab 직접 target — 이중 window/잘못된 front window 방지
osascript <<EOF
tell application "Terminal"
    activate
...
echo "   bounds:   $BOUNDS (모태 invasion INTEL 좌표)"
echo "   refresh:  ${REFRESH_SEC}s"
echo "   override: POLARIS_DASH_PROFILE / POLARIS_DASH_BOUNDS / POLARIS_DASH_REFRESH"
