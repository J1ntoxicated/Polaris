...
def test_total_demo_equity_130000() -> None:
    """Total = OKX 79K + Capital 51K = 130K."""
    assert TOTAL_DEMO_STARTING_EQUITY_USD == 130_000.0
    assert demo_starting_equity_total() == 130_000.0
...
def test_env_override_okx(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Total picks up the override too.
    assert demo_starting_equity_total() == 100_000.0 + CAPITAL_DEMO_STARTING_EQUITY_USD


def test_env_override_total(monkeypatch: pytest.MonkeyPatch) -> None:
    """Per-venue env unset → POLARIS_DEMO_STARTING_EQUITY_TOTAL wins."""
    ...
    assert demo_starting_equity_total() == 200_000.0
...
def test_env_override_total_splits_venues_proportionally(
    ...
    # Sum invariant — the source of truth for the F12 bug.
    assert math.isclose(okx + cap, total, abs_tol=1e-6)
    assert math.isclose(total, 200_000.0)
...
def test_env_override_capital_plus_total_okx_fills_gap(
    ...
    assert math.isclose(total, 150_000.0)
    assert math.isclose(okx + cap, total, abs_tol=1e-6)
...
def test_env_override_all_three_set_per_venue_wins(
    ...
    assert math.isclose(total, 100_000.0)
    assert math.isclose(okx + cap, total, abs_tol=1e-6)
...
def test_sum_invariant_holds_in_every_envvar_configuration(
    ...
    """Property: ``okx + capital == total`` for every env configuration.

    This is the codex F12 round 2 acceptance criterion — the bug class
    we're closing is "renderer split mismatches dashboard total"."""
    import math
    ...
    for okx_env, cap_env, total_env, label in cases:
        ...
        total = demo_starting_equity_total()
        assert math.isclose(okx + cap, total, abs_tol=1e-6), (
            f"sum invariant failed for {label}: okx={okx} cap={cap} total={total}"
        )
...
def test_starting_capital_resolves_from_constants() -> None:
    """``STARTING_CAPITAL`` (module constant) must equal the SSOT total."""
    from polaris.scripts.dashboard import snapshot as snap_mod

    assert snap_mod.STARTING_CAPITAL == TOTAL_DEMO_STARTING_EQUITY_USD


def test_dashboard_snapshot_zero_db_uses_total_base(tmp_path: Path) -> None:
    """A missing DB still returns a snapshot with the SSOT total as base."""
    from polaris.scripts.dashboard.snapshot import collect_snapshot

    snap = collect_snapshot(db_path=tmp_path / "absent.sqlite")
    assert snap.starting_capital == TOTAL_DEMO_STARTING_EQUITY_USD
    ...
    assert snap.equity_now == TOTAL_DEMO_STARTING_EQUITY_USD
...
def test_dashboard_snapshot_dd_calculated_from_correct_base(
    ...
    # Equity ~ 130K - 3.5K = ~126.5K
    assert snap.starting_capital == TOTAL_DEMO_STARTING_EQUITY_USD
    # DD must be small (-3.5K against 130K base ≈ ~2.69%, NOT 35% on 10K base).
