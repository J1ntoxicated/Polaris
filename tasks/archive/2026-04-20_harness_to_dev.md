...
## [2026-04-20 19:15 AEST] MSG-TICKER-LEARNER-AMPLIFY-ONLY PENDING — [P0] 🟩 HARNESS

### 북극성 위반 확정 (feedback_no_defensive_param_dampen)
`invasion/ops/ticker_learner.py:104` 에 low-WR ticker 대상 `mult = round(max(0.3, wr*2), 1)` → **0.3x~0.6x REDUCE sizing 발생 중**.

**실 증거 (T6 London 18:00-18:30 로그)**:
- `LEARN REDUCE USD/CHF WR=21% → 0.4x`
- `LEARN REDUCE France 40 WR=13% → 0.3x`
- `LEARN REDUCE Germany 40 WR=30% → 0.6x`

**영향**: DAX/FTSE/CAC 등 유럽 지수 진입 0건 원인 중 하나 (0.3x sizing → min_size 미달 → "No strategy match"). 북극성 "공격적 상시 수익, dampen 금지" 직접 위반.

**전례**: 04-19 `17090b46` commit 에서 `adaptive_tuner.py:202` 에 동일 패턴 `max(0.5, min(1.5, mult))` → `max(1.0, min(1.5, mult))` amplify-only 적용 완료. ticker_learner 는 누락.

### 수정 스펙
`invasion/ops/ticker_learner.py:98-106` 블록 전체를 amplify-only 로 재작성:

```python
# 1. Sizing: amplify-only (북극성 feedback_no_defensive_param_dampen)
if wr >= _BOOST_WR:
    # WR 65%→1.3x, 80%→1.8x, 90%→2.2x
    mult = round(1.0 + (wr - 0.5) * 4.0, 1)
    mult = max(1.2, min(2.5, mult))
else:
    mult = 1.0  # low WR 은 중립 (dampen 금지)
```

**변경 내역**:
- `elif wr < _PENALIZE_WR` 분기 삭제
- `max(0.3, wr*2)` 삭제
- line 111 `_tag = "BOOST" if mult > 1.0 else "REDUCE"` → `_tag = "BOOST"` (REDUCE 케이스 제거)
- `_PENALIZE_WR` 상수가 다른 곳에서도 쓰이는지 grep 필요. 안 쓰이면 제거, 쓰이면 direction bias 등 별도 용도라 유지.

### 검증 요구
1. grep `_PENALIZE_WR` — sizing 외 용도 확인
2. `python3 -c "import invasion.ops.ticker_learner"` OK
3. smoke 5-step
4. 기존 `tests/ops/test_ticker_learner*` 있으면 REDUCE 경로 테스트 업데이트

### Commit msg
```
fix(msg-ticker-learner-amplify-only jin p0): REDUCE 0.3-0.6x 제거 (북극성 dampen 금지)
```

### Action Required
commit 후 `[RESTART-REQUEST]` push. Harness 가 `bash start.sh` 실행.
