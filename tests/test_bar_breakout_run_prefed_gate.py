...
pre-fix shape), the module's own loosened ``DONCHIAN_WINDOW`` (40->20 in
...
fixed window). In VIRTUAL mode (window=20) the in-module recompute always
...
ALWAYS finite (as production.py feeds it), and assert:
  (a) VIRTUAL mode fires on a series that breaks the last-20-bar high but NOT
...
def _series_breaks_20_not_40() -> list[float]:
    """65 bars: a flat 40-bar anchor, then a deep 20-bar dip, then a rally that
    clears the last-20-bar high but stays under the bar-40-ago high."""
    closes = [100.0] * 40  # bar-40-ago high anchor = 100.3 (high = close+0.3)
    closes += [85.0] * 20  # deep dip -> resets the 20-bar high much lower
    closes += [90.0]  # final close: > 20-bar prior high (85.3), < 40-bar (100.3)
    return closes
...
def test_virtual_mode_fires_with_prefed_40bar_finite() -> None:
    ...
    40-bar), yet VIRTUAL mode must still fire off the loosened 20-bar window —
    proving the in-module recompute runs, not the stale pre-fed reuse."""
    closes = _series_breaks_20_not_40()
    bars = _bars(closes)
    ...
    assert sig is not None, (
        "VIRTUAL (20-bar loosened) must fire even when the pre-fed "
