...
_EARNINGS_PROXIMITY_SHADOW_SQL = (
...
def log_earnings_proximity_shadow(
    ...
    *,
    ...
    if conn is None or not symbol:
        return 0
    args = (symbol, cycle_ts, days_to_earnings, hour, surprise_pct, cycle_ts)
    try:
        if db_writer is not None and dbwriter_enabled():
            def _job(
                c: sqlite3.Connection,
                sql: str = _EARNINGS_PROXIMITY_SHADOW_SQL, a: tuple[object, ...] = args,
            ) -> None:
                c.execute(sql, a)

            db_writer.submit(_job, label="earnings_proximity_shadow")
            return 1
        cur = conn.execute(_EARNINGS_PROXIMITY_SHADOW_SQL, args)
