...
def test_okx_fill_always_taker_default_ignores_exectype() -> None:
    """FIX2 reverted (#87 defer): the production OKX open path
    (adapter.fetch_order → order-details) never supplies ``execType``, so reading
    it was a no-op that always fell through to taker. The misleading per-fill
    maker/taker derivation is removed — ``fee_usd`` is the REAL taker schedule
    (10 bps) for EVERY fill, even one that (theoretically) carries ``execType=='M'``.
    Per-fill maker/taker is deferred to #87 (order-mode wiring). This proves
    execType is no longer read."""
    payload = {
        "ordId": "100",
        "instId": "BTC-USDT",
        "side": "buy",
        ...
        "accFillSz": "0.001",
        "avgPx": "60000",
        "execType": "M",  # would-be maker — MUST be ignored now (taker default)
        "state": "filled",
        "uTime": "1762476225678",
    }
    f = normalize_okx_fill(payload, strategy_id="weekend_thin_book_flush_maker")
    # 10 bps real taker of the $60 notional = $0.06 (NOT 8 bps maker = $0.048).
    assert f.fee_usd == pytest.approx(f.quote_qty * 0.001, rel=1e-9)
