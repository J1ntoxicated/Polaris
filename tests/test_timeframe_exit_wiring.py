...
#1 timeframe-blind ruler: a 1H tsmom position is now trailed on the 1H ATR
...
def _trade(position_id: str, strategy: str = "tsmom") -> SimulatedTrade:
...
async def test_1h_strategy_trail_uses_1h_atr(memdb: sqlite3.Connection) -> None:
    ...
    _seed_bars(memdb, interval="1H", n=20, band=2.0, close=100.0,
               end_ts=NOW - 60)
    _seed_position(memdb, position_id="pos-1h", strategy="tsmom")
    state = ProdLoopState()
...
async def test_anchor_denominates_persisted_excursion(
    ...
    _seed_bars(memdb, interval="1H", n=20, band=2.0, close=100.0,
               end_ts=NOW - 60)
    _seed_position(
        memdb, position_id="pos-anchor", strategy="tsmom",
...
async def test_legacy_null_anchor_falls_back_to_current_tf_atr(
    ...
    _seed_bars(memdb, interval="1H", n=20, band=2.0, close=100.0,
               end_ts=NOW - 60)
    _seed_position(memdb, position_id="pos-legacy", strategy="tsmom")
    state = ProdLoopState()
...
def test_load_rows_exposes_anchor_and_missing_flag(
    ...
    _seed_position(
        memdb, position_id="pos-a", strategy="tsmom",
        entry_atr_pct=0.03, entry_atr_timeframe="1H",
    )
    _seed_position(memdb, position_id="pos-b", strategy="tsmom")
    rows = {
...
async def test_open_path_stamps_entry_anchor(memdb: sqlite3.Connection) -> None:
    ...
    _seed_bars(memdb, interval="1H", n=20, band=2.0, close=100.0)
    sig = RawSignal(
        signal_id="sig-anchor", strategy_id="tsmom", symbol="BTC-USDT",
...
async def test_open_path_stamps_null_when_no_bars(
    ...
    reset_process_fence()
    sig = RawSignal(
        signal_id="sig-nobars", strategy_id="tsmom", symbol="ETH-USDT",
        side="long", strength=0.8, sizing_hint=0.05, ttl_bars=10,
        thesis_tag="t", correlation_group="spot_cross_sectional_momo",
    )
    state = ProdLoopState()
...
def test_close_pnl_r_uses_entry_anchor(memdb: sqlite3.Connection) -> None:
    # Tight 1m bars (atr 0.001) would put -2.0 at -10R (clamped); the anchor
    # (0.04) makes it the honest -0.25R.
    _seed_bars(memdb, interval="1m", n=14, band=0.05, close=100.0)
    _seed_position(
        memdb, position_id="pos-close", strategy="tsmom",
        entry_atr_pct=0.04, entry_atr_timeframe="1H",
    )
    trade = _trade("pos-close")
    pnl_r, _pnl_usd, exit_price = real_pnl_r_from_fills(
        memdb, trade=trade, exit_price_override=98.0,
    )
    assert exit_price == 98.0
    assert pnl_r == pytest.approx(-2.0 / (100.0 * 0.04 * 2.0))


def test_close_pnl_r_legacy_null_anchor_unchanged(
    memdb: sqlite3.Connection,
) -> None:
    _seed_bars(memdb, interval="1m", n=14, band=0.25, close=100.0)  # atr 0.005
    _seed_position(memdb, position_id="pos-close-l", strategy="tsmom")
    trade = _trade("pos-close-l")
    pnl_r, _pnl_usd, _exit = real_pnl_r_from_fills(
        memdb, trade=trade, exit_price_override=99.0,
    )
    # Legacy denominator: 1m bars → -1.0 / (100×0.005×2) = -1.0R.
...
def test_close_excursion_uses_entry_anchor(memdb: sqlite3.Connection) -> None:
    _seed_bars(memdb, interval="1m", n=14, band=0.05, close=100.0)
    _seed_position(
        memdb, position_id="pos-exc", strategy="tsmom",
        entry_atr_pct=0.04, entry_atr_timeframe="1H",
    )
    memdb.execute(
